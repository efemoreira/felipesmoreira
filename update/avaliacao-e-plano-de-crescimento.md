...
| 5 | Sem `Content-Security-Policy`, `frame-ancestors`/`X-Frame-Options`, `Permissions-Policy`. E **sem `Cache-Control` nas telas HTML do painel** — só `estudio.php` e as APIs mandam `no-store`; a lista de pessoas pode ficar no cache do navegador de um celular emprestado | `layout.php` (onde `X-Robots-Tag` já sai) | — |
