...
| 7 (manutenção) | 12/09 | componentização 4→7 (home e legais em `features/`, `entregarArte()` comum, `CONTRIBUTING.md`) · painel: form inline zero e `style=` 158→82 com teto · Tailwind fica pelo Estúdio (achado corrigido) | **8,6** | — |
| 6 (os próximos) | 12/09 | aulas 4→7 (texto editável por patch) · semana/mês 5→7 (metas, tarefas, importar) · tokens 5→8 (`contrato/tema`) · escala sem conta já existia (6→7 ao ler certo) | **8,5** | — |
| 5 (terceira avaliação) | 12/09 | aprovar da ficha · vazio com botão em toda tela · erros de produção na Manutenção (observabilidade 4→6) · `painel.js` + **CSP com nonce** (segurança) · ficha do fato com um botão e data legível · medidor do dado · `/programacao` vazio com ação · primeiros passos cobrem agenda/estúdio | **8,2** | — |
