...
type OpcoesTexto = {
  ...
  espaco?: number; // letter-spacing em px
};

function escrever(ctx: CanvasRenderingContext2D, txt: string, x: number, y: number, o: OpcoesTexto) {
  ctx.save();
  ctx.font = o.font;
  ctx.fillStyle = o.cor;
...
function retangulo(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r = 0) {
  ctx.beginPath();
...
function icone(ctx: CanvasRenderingContext2D, nome: string, x: number, y: number, tamanho: number, cor: string) {
