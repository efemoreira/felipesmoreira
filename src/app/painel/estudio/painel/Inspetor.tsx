  ...
  if (!camada) {
    return (
      <div className="px-4 py-8 text-center">
        <p className="text-sm leading-relaxed text-[color:var(--e-t4)]">
          Escolha uma camada no palco ou na lista à esquerda para editar.
        </p>
        <p className="mt-4 text-[11px] leading-relaxed text-[color:var(--e-t4)]">
  ...
  return (
    <button
      ...
      className={`w-full rounded-md px-3 py-2 text-xs font-medium transition ${
        destaque
          ? "bg-[#FFCB05] text-[color:var(--e-tinta)] hover:bg-[#ffd63a]"
          : risco
            ? "border border-red-400/40 text-red-200 hover:border-red-400 hover:bg-red-500/10"
            : "border border-[color:var(--e-b1)] text-[color:var(--e-t2)] hover:border-[color:var(--e-b2)] hover:text-[color:var(--e-t1)]"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="flex flex-col">
      <Secao titulo={`${camadas.length} camadas`}>
        <p className="text-[11px] leading-relaxed text-[color:var(--e-t4)]">{tipos.join(" · ")}</p>
        <Deslizante
          rotulo="Opacidade"
        ...
        <>
          ...
          {esmaecer.saida === "cor" && (
            <>
              ...
              <p className="text-[11px] leading-snug text-[color:var(--e-t4)]">
      ...
      <div className="flex flex-wrap gap-1.5 pt-1">
        {FUNDOS.map((f) => (
          <button
            ...
            className="h-8 flex-1 rounded border border-[color:var(--e-b1)] text-[10px] text-[color:var(--e-t1)] hover:border-[#FFCB05]"
          >
            {f.nome}
          </button>
        ))}
...
function PainelTarja({ camada, alterar }: { camada: CamadaTexto; alterar: Alterar }) {
  ...
  return (
    <Secao titulo="Acabamento do ==assim==" aberta={false}>
      {!usa && (
        <p className="text-[11px] leading-snug text-[color:var(--e-t4)]">
          Escreva <code className="text-[color:var(--e-acento)]">==uma palavra==</code> no conteúdo para
