...
- A semana corrente vai de **domingo a sábado** por padrão, no fuso do Ceará; a capa da programação (`agenda.php`, campo `inicioSemana`) pode abri-la na **segunda**. `semana_de()`/`proxima_semana_de()`/`dia_de()` (PHP) e `semanaDe()`/`proximaSemanaDe()`/`diaDe()` (TS) têm de concordar — `testes/contrato/semana.test.ts` prende o par. Quem recorta por conta da programação passa `comeco_da_semana(agenda_atual())`/`comecoDaSemana(agenda)`; `kit`, `oficina` e `leituras` usam a semana como **chave** e ficam no domingo de propósito. O período da capa é calculado daí quando o campo do painel está vazio, e na página ele acompanha o recorte (`periodoDoRecorte()`). A capa também guarda `grupo` — o link do grupo de quem quer ajudar na organização, que vira o convite embaixo da lista em `/programacao`.
